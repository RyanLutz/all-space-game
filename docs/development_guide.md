...
| 18 | 🔲 | Star System — Phase 1: Data + Generation + LOD 0 | `feature_spec-star_system.md` | Sonnet | `StarRecord.gd`, `StarRegistry.gd` skeleton, procedural catalog from seed (spiral-arm distribution), `MultiMeshInstance3D` for galactic-scale point rendering. `world_config.json` galaxy fields. All four `StarRegistry.*` PerformanceMonitor metrics wired. Gate: deterministic catalog; all stars in one draw call at galactic zoom. |
| 19 | 🔲 | Star System — Phase 2: Screen-Space Glow Shader (LOD 1) | `feature_spec-star_system.md` | **Opus** | `star_screen_pass.gdshader` — SubViewport full-screen pass; world pos → NDC via camera VP matrix uniform; radial glow disc per star; no depth test. Composites behind all scene geometry. `_update_shader_uniforms()` packs visible star list each frame. LOD 0→1 state transition. Gate: mid-range stars as screen-space glow with no Z-fighting; clean transition from MultiMesh. |
