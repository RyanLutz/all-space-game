...
| 24 | ✅ | StarField S2 — Galactic Map UI Layer *(superseded)* | `spec/feature_spec-star_field_2.md` | **Haiku** | *Old 2D canvas map (`GalacticMap.gd` / `GalacticMap.tscn`) deleted. Replaced by 3D galaxy map in step 25b — see `spec/feature_spec-galaxy_map.md`.* |
